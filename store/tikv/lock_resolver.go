...
func (lr *LockResolver) BatchResolveLocks(bo *Backoffer, locks []*Lock, loc RegionVerID) (bool, error) {
	...
	req := tikvrpc.NewRequest(tikvrpc.CmdResolveLock, unsafe.Pointer(&kvrpcpb.ResolveLockRequest{TxnInfos: listTxnInfos}))
	startTime = time.Now()
...
func (lr *LockResolver) getTxnStatus(bo *Backoffer, txnID uint64, primary []byte) (TxnStatus, error) {
	...
	var status TxnStatus
	req := tikvrpc.NewRequest(tikvrpc.CmdCleanup, unsafe.Pointer(&kvrpcpb.CleanupRequest{
		Key:          primary,
		StartVersion: txnID,
	}))
	for {
...
func (lr *LockResolver) resolveLock(bo *Backoffer, l *Lock, status TxnStatus, cleanRegions map[RegionVerID]struct{}) error {
	...
	for {
		...
		req := tikvrpc.NewRequest(tikvrpc.CmdResolveLock, unsafe.Pointer(lreq))
		resp, err := lr.store.SendReq(bo, req, loc.Region, readTimeoutShort)
		if err != nil {
			return errors.Trace(err)
		}
