...
func (e *AnalyzeIndexExec) fetchAnalyzeResult(ranges []*ranger.Range, isNullRange bool) error {
	...
	if e.isCommonHandle && e.idxInfo.Primary {
		kvReqBuilder = builder.SetCommonHandleRangesForMultiTbl(e.ctx.GetSessionVars().StmtCtx, e.tableID.CollectIDs, ranges)
	} else {
		kvReqBuilder = builder.SetIndexRangesForMultiTbl(e.ctx.GetSessionVars().StmtCtx, e.tableID.CollectIDs, e.idxInfo.ID, ranges)
	}
