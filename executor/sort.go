...
func (e *SortExec) fetchRowChunks(ctx context.Context) error {
	fields := e.retTypes()
	e.rowChunks = chunk.NewList(fields, e.initChunkSize, e.maxChunkSize)
...
func (e *SortExec) buildKeyChunks() error {
	e.keyChunks = chunk.NewList(e.keyTypes, e.initChunkSize, e.maxChunkSize)
...
func (e *TopNExec) loadChunksUntilTotalLimit(ctx context.Context) error {
	...
	if e.totalLimit > e.maxChunkSize {
		initChunkSize = e.maxChunkSize
	}
	e.rowChunks = chunk.NewList(e.retTypes(), initChunkSize, e.maxChunkSize)
	e.rowChunks.GetMemTracker().AttachTo(e.memTracker)
	e.rowChunks.GetMemTracker().SetLabel("rowChunks")
	for e.rowChunks.Len() < e.totalLimit {
		srcChk := e.children[0].newChunkWithCapacity(initChunkSize)
...
func (e *TopNExec) doCompaction() error {
	newRowChunks := chunk.NewList(e.retTypes(), e.initChunkSize, e.maxChunkSize)
	...
	if e.keyChunks != nil {
		newKeyChunks := chunk.NewList(e.keyTypes, e.initChunkSize, e.maxChunkSize)
		for _, rowPtr := range e.rowPtrs {
