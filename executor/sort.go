...
func (e *SortExec) buildKeyChunks() error {
	e.keyChunks = chunk.NewList(e.keyTypes, e.chunkCap, e.maxChunkSize)
...
func (e *TopNExec) loadChunksUntilTotalLimit(ctx context.Context) error {
	...
	for e.rowChunks.Len() < e.totalLimit {
		srcChk := e.children[0].newChunkWithCapacity(chunkCap)
...
func (e *TopNExec) executeTopN(ctx context.Context) error {
	...
	if e.keyChunks != nil {
		childKeyChk = chunk.NewChunkWithCapacity(e.keyTypes, e.chunkCap)
...
func (e *TopNExec) doCompaction() error {
	newRowChunks := chunk.NewList(e.retTypes(), e.chunkCap, e.maxChunkSize)
	...
	if e.keyChunks != nil {
		newKeyChunks := chunk.NewList(e.keyTypes, e.chunkCap, e.maxChunkSize)
		for _, rowPtr := range e.rowPtrs {
